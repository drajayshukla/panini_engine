...
class Varna:
    def __init__(self, raw_unit):
        ...
        # 1. Vowel Detection (Sutra 1.1.1/1.1.2 context)
        # Checks if char is in the vowel list OR if it's a Pluta vowel (contains '३')
        self.is_vowel = any(v in raw_unit for v in INDEPENDENT_VOWELS) or '३' in raw_unit

        # 2. Nasal Detection (Sutra 1.1.8 context)
        self.is_anunasika = 'ँ' in raw_unit

        # 3. Consonant Detection (Sutra 1.1.7 context)
        # CRITICAL FIX: Defined as "Not a Vowel" AND "Contains Virama" (since we use Halant-based storage)
