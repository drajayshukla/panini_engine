import streamlit as st
import json
import os
import random
import pandas as pd
from collections import Counter

# --- १. पेज सेटअप ---
st.set_page_config(page_title="Vibhakti Diagnostic Engine", layout="wide", page_icon="🔬")


@st.cache_data
def load_shabd_data():
    file_path = os.path.join("data", "shabdroop.json")
    try:
        if not os.path.exists(file_path): return []
        with open(file_path, "r", encoding="utf-8") as file:
            return json.load(file)
    except:
        return []


# --- २. Sanskrit-Aware Suffix Matcher ---
def sanskrit_match(word, suffix):
    if not suffix or suffix == "--सभी दिखाएं--": return True
    word = str(word).strip()

    vowel_to_matra = {
        'आ': 'ा', 'इ': 'ि', 'ई': 'ी', 'उ': 'ु', 'ऊ': 'ू',
        'ऋ': 'ृ', 'ॠ': 'ॄ', 'ए': 'े', 'ऐ': 'ै', 'ओ': 'ो', 'औ': 'ौ'
    }

    if suffix.startswith('अ'):
        core = suffix[1:]
        if not word.endswith(core): return False
        pos = len(word) - len(core) - 1
        if pos < 0: return False
        return '\u0915' <= word[pos] <= '\u0939'

    for v, m in vowel_to_matra.items():
        if suffix.startswith(v):
            if word.endswith(m + suffix[1:]): return True

    return word.endswith(suffix)


# --- ३. मास्टर लॉजिक एवं क्रम ---
LOGIC_RULES = {
    "प्रथमा एकवचन": ["ः", "न्", "म्", "ा", "ी", "ू", "ऋ", "क्", "ट्", "प्", "त्"],
    "प्रथमा द्विवचन": ["आ", "ई", "औ", "ए", "ौ", "े", "ी"],
    "प्रथमा बहुवचन": ["ाः", "यः", "वः", "णि", "सि", "अः", "हः", "धः", "शः", "तः", "नः"],
    "द्वितीया बहुवचन": ["आन्", "ईन्", "ून्", "ॄन्", "ः", "ईः", "ऊः", "अः", "ाः"],
    "तृतीया एकवचन": ["एण", "णा", "ना", "या", "त्रा", "सा", "वा", "ा", "ता", "धा", "भा"],
    "चतुर्थी एकवचन": ["आय", "ये", "वे", "त्रे", "यै", "स्मै", "भ्यम्", "ते", "ने", "से", "ए"],
    "षष्ठी एकवचन": ["स्य", "अः", "ओः", "आः", "याः", "तुः", "सः", "तः", "नः", "चः", "जः"],
    "सप्तमी एकवचन": ["ए", "औ", "रि", "ति", "नि", "वि", "यि", "याम्", "वाम्", "इ", "षि", "धि"]
}

VIBHAKTI_ORDER = {
    "प्रथमा एकवचन": 1, "प्रथमा द्विवचन": 2, "प्रथमा बहुवचन": 3,
    "द्वितीया बहुवचन": 6, "तृतीया एकवचन": 7, "चतुर्थी एकवचन": 10,
    "षष्ठी एकवचन": 16, "सप्तमी एकवचन": 19
}


def main():
    st.title("🔬 Clinical Vibhakti Diagnostic Engine")
    st.caption("विभक्ति क्रमानुसार संतुलित प्रदर्शन (Max 50 results total)")

    data = load_shabd_data()
    if not data:
        st.error("डेटाबेस (data/shabdroop.json) अप्राप्त है।")
        st.stop()

    all_symptoms = sorted(list(set([s for rules in LOGIC_RULES.values() for s in rules])))
    selected_symptom = st.sidebar.selectbox("प्रत्यय लक्षण चुनें:", ["--सभी दिखाएं--"] + all_symptoms)

    # --- ४. संतुलित डायग्नोस्टिक प्रोसेसिंग ---
    matches = []
    category_counter = Counter()
    random.shuffle(data)

    # प्रति श्रेणी सीमा तय करना (50/8 ≈ 6 प्रति श्रेणी ताकि विविधता बनी रहे)
    limit_per_category = 7

    for entry in data:
        if len(matches) >= 50: break  # कुल ५० की सीमा

        raw_forms = entry.get("forms", "").split(";")
        if len(raw_forms) < 21: continue

        mapping = {
            "प्रथमा एकवचन": raw_forms[0], "प्रथमा द्विवचन": raw_forms[1], "प्रथमा बहुवचन": raw_forms[2],
            "द्वितीया बहुवचन": raw_forms[5], "तृतीया एकवचन": raw_forms[6], "चतुर्थी एकवचन": raw_forms[9],
            "षष्ठी एकवचन": raw_forms[15], "सप्तमी एकवचन": raw_forms[18]
        }

        for vibhakti, roop in mapping.items():
            if category_counter[vibhakti] >= limit_per_category: continue

            for p in LOGIC_RULES[vibhakti]:
                if sanskrit_match(roop, p):
                    if selected_symptom == "--सभी दिखाएं--" or sanskrit_match(roop, selected_symptom):
                        matches.append({
                            "order": VIBHAKTI_ORDER[vibhakti],
                            "शब्द": entry.get("word"),
                            "विभक्ति": vibhakti,
                            "रूप": roop,
                            "लक्षण": p
                        })
                        category_counter[vibhakti] += 1
                        break
            if len(matches) >= 50: break

    # --- ५. सॉर्टिंग एवं प्रदर्शन ---
    if matches:
        sorted_examples = sorted(matches, key=lambda x: x['order'])
        df = pd.DataFrame(sorted_examples)
        st.table(df[["शब्द", "विभक्ति", "रूप", "लक्षण"]])
        st.info(f"💡 संतुलित प्रदर्शन: कुल {len(matches)} उदाहरण क्रमानुसार प्रदर्शित हैं।")
    else:
        st.warning(f"लक्षण '{selected_symptom}' के लिए कोई मेल नहीं मिला।")


if __name__ == "__main__":
    main()