...
@st.cache_data
def load_shabd_data():
    # डॉक्टर साहब द्वारा निर्देशित पाथ: data/shabdroop.json
...
# --- २. Robust Sanskrit-Aware Suffix Matcher ---
def sanskrit_match(word, suffix):
    """संस्कृत के अदृश्य 'अ', मात्राओं और व्यञ्जनान्त लोप को पहचानने वाला लॉजिक"""
    ...
    # १. 'अ' अंत (Inherent A) की जांच - उदा. 'अः' matches 'रामः'
    if suffix.startswith('अ'):
        core = suffix[1:]
        if not word.endswith(core): return False
        pos = len(word) - len(core) - 1
        if pos < 0: return False
        # यदि विसर्ग/प्रत्यय से पहले शुद्ध व्यंजन है, तो वह 'अ' अंत है
        return '\u0915' <= word[pos] <= '\u0939'

    # २. स्वरों को मात्राओं में बदलकर जांचना
    for v, m in vowel_to_matra.items():
        if suffix.startswith(v):
            if word.endswith(m + suffix[1:]): return True

    # ३. सामान्य अंत (विसर्ग, हलन्त आदि)
    return word.endswith(suffix)


# --- ३. परिष्कृत एवं Robust मास्टर लॉजिक ---
# यहाँ 'स्वर' को उनकी मात्राओं के रूप में और व्यञ्जनान्त 'Signatures' को शामिल किया गया है
LOGIC_RULES = {
    "प्रथमा एकवचन": [
        "ः", "न्", "म्", "ा", "ी", "ू", "ऋ",
        "क्", "ट्", "प्", "त्"
    ...
    "तृतीया एकवचन": [
        "एण", "णा", "ना", "या", "त्रा", "सा", "वा", "ा",
        "ता", "धा", "भा"
    ],
    "चतुर्थी एकवचन": [
        "आय", "ये", "वे", "त्रे", "यै", "स्मै", "भ्यम्",
        "ते", "ने", "से", "ए"
    ],
    "षष्ठी एकवचन": [
        "स्य", "अः", "ओः", "आः", "याः", "तुः", "सः",
        "तः", "नः", "चः", "जः"
    ],
    "सप्तमी एकवचन": [
        "ए", "औ", "रि", "ति", "नि", "वि", "यि", "याम्", "वाम्",
        "इ", "षि", "धि"
    ]
}
...
# --- ४. मुख्य एप्लीकेशन लॉजिक ---
def main():
    st.title("🔬 Clinical Vibhakti Diagnostic Engine")
    st.caption("विभक्ति क्रमानुसार संतुलित प्रदर्शन (Max 5 per category) | केवल व्याकरणिक विश्लेषण")
    ...
    # लक्षणों की मास्टर लिस्ट (Unique Suffixes)
    ...
    selected_symptom = st.sidebar.selectbox(
        ...
        ["--सभी दिखाएं--"] + all_symptoms
    )

    # ५. संतुलित डायग्नोस्टिक प्रोसेसिंग
    matches = []
    category_counter = Counter()

    # रैंडम सैंपलिंग के लिए डेटा शफल
    random.shuffle(data)
        ...
        for vibhakti, roop in mapping.items():
            ...
            for p in patterns:
                if sanskrit_match(roop, p):
                    # यदि यूजर ने कोई स्पेसिफिक लक्षण चुना है
    ...
    # ६. क्रमानुसार सॉर्टिंग एवं प्रदर्शन
    if matches:
        sorted_examples = sorted(matches, key=lambda x: x['order'])
        df = pd.DataFrame(sorted_examples)

        # अर्थ कॉलम हटाकर केवल शुद्ध व्याकरणिक डेटा
        st.table(df[["शब्द", "विभक्ति", "रूप", "लक्षण"]])
        st.info(f"💡 संतुलित प्रदर्शन: प्रत्येक उपलब्ध श्रेणी से अधिकतम 5 उदाहरण। कुल {len(matches)} रूप प्रदर्शित हैं।")
    else:
