#pages/07_Pratyahara_Lab.py
import streamlit as st
import json
import os
from core.pratyahara_engine import PratyaharaGenerator

# --- १. पाणिनीय ४२ प्रत्याहारों की प्रामाणिक सूची ---
# इसे हलन्त के साथ रखा गया है ताकि मिलान (Matching) सटीक हो
PANINI_PRATYAHARAS = [
    "अक्", "अच्", "अट्", "अण्", "अम्", "अश्", "अल्",
    "इक्", "इच्", "इण्", "उक्", "एङ्", "एच्", "ऐच्",
    "खय्", "खर्", "ङम्", "चय्", "चर्", "छव्", "जश्",
    "झय्", "झर्", "झल्", "झश्", "झष्", "तङ्", "तव्",
    "नम", "मय्", "यञ्", "यण्", "यम्", "यय्", "यर्",
    "वल्", "वश्", "शल्", "शर्", "हल्", "हश्", "र"
]

# --- २. पेज कॉन्फ़िगरेशन ---
st.set_page_config(page_title="Pratyahara Lab", layout="wide", page_icon="🎼")
st.title("🎼 प्रत्याहार निर्माण प्रयोगशाला (SK2 Lab)")
st.caption("सूत्र: आदिरन्त्येन सहेता (१.१.७१) - प्रत्याहार जनरेटर")


# --- ३. डेटा लोडर ---
@st.cache_data
def load_shiva_sutras():
    path = 'data/shiva_sutras.json'
    if os.path.exists(path):
        with open(path, 'r', encoding='utf-8') as f:
            return json.load(f)['shiva_sutras']
    return []


shiva_sutras = load_shiva_sutras()

# ४. आदि और अन्त्य वर्णों का संग्रह
all_adis = []
all_its = []
for s in shiva_sutras:
    all_adis.extend(s['varnas'])
    all_its.append(s['it_varna'])

# --- ५. यूआई लेआउट ---
col1, col2 = st.columns([1, 2])

with col1:
    st.subheader("🛠️ निर्माण पैरामीटर्स")
    mode = st.radio("मोड चुनें:", ["Standard (पाणिनीय)", "Custom (कृत्रिम)"])

    if mode == "Standard (पाणिनीय)":
        selected_p = st.selectbox("अष्टाध्यायी में प्रयुक्त प्रत्याहार चुनें:", options=sorted(PANINI_PRATYAHARAS))

        if selected_p == "र":
            adi_val, it_val = "र", "ल"
        else:
            adi_val = selected_p[0]
            # 'अच्' से 'च्' निकालने के लिए
            it_char = selected_p[1:]
            # माहेश्वर सूत्रों की it_varna लिस्ट में सटीक मैच खोजना (जैसे 'च्' या 'श्')
            it_val = next((it for it in all_its if it.startswith(it_char)), all_its[0])
    else:
        adi_val = st.selectbox("आदि वर्ण (Start):", options=all_adis)
        it_val = st.selectbox("अन्त्य इत् (End):", options=all_its)

# ६. प्रत्याहार जनरेशन
result_varnas = PratyaharaGenerator.generate(adi_val, it_val, shiva_sutras)

# नाम को 'Match' करने के लिए क्लीनअप (बिना हलन्त वाला वर्जन)
# ताकि 'अच्' को 'अच' से भी मैच किया जा सके
display_name = f"{adi_val}{it_val}".replace("्", "")
full_it_name = f"{adi_val}{it_val}"

# --- ७. मुख्य डिस्प्ले ---
with col2:
    st.header(f"💠 विश्लेषण: {full_it_name}")

    # पाणिनीय वैधता चेक (Clinical Validation)
    # हम 'अच्' (selected_p) और 'अच' (display_name) दोनों को चेक कर रहे हैं
    is_panini = display_name in PANINI_PRATYAHARAS or \
                full_it_name in PANINI_PRATYAHARAS or \
                (display_name == "र" and adi_val == "र")

    if is_panini:
        st.success("✅ यह एक **प्रामाणिक पाणिनीय प्रत्याहार** है।")
    else:
        st.warning("⚠️ यह एक **कृत्रिम (Artificial) प्रत्याहार** है।")

    if not result_varnas:
        st.error(f"क्रम त्रुटि: '{adi_val}' के बाद '{it_val}' नहीं आता।")
    else:
        st.subheader("📚 शामिल वर्ण (Varnas):")
        varna_html = "".join([
                                 f"<div style='display:inline-block; background-color:#e1f5fe; border-radius:8px; padding:10px 20px; margin:5px; font-size:1.5rem; border:1px solid #01579b; font-weight:bold; color:#01579b;'>{v}</div>"
                                 for v in result_varnas])
        st.markdown(varna_html, unsafe_allow_html=True)
        st.info(f"कुल वर्णों की संख्या: **{len(result_varnas)}**")

# --- ८. माहेश्वर सूत्र संदर्भ ---
st.divider()

with st.expander("🕉️ माहेश्वर सूत्र संदर्भ तालिका"):
    st.table([{"क्रम": s['id'], "सूत्र": s['sutra'], "वर्ण": ", ".join(s['varnas']), "इत्": s['it_varna']} for s in
              shiva_sutras])