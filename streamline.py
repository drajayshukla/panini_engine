...
def repair_test_page():
    print("🔧 Repairing pages/1_test.py and Logic linkage...")

    # 1. FIX: pages/1_test.py (Add Path Hack)
    test_page_path = Path("pages/1_test.py")
    test_page_code = r'''import streamlit as st
import sys
import os
...
'''
    with open(test_page_path, "w", encoding="utf-8") as f:
        f.write(test_page_code)
    print(f"✅ Fixed: {test_page_path} (Added sys.path hack)")

    # 2. VERIFY: logic/__init__.py
    # Ensures 'from logic.subanta_processor' works
    init_path = Path("logic/__init__.py")
    init_code = "from .subanta_processor import SubantaProcessor\nfrom .sandhi_processor import SandhiProcessor"
    
    # Force rewrite to be sure
    with open(init_path, "w", encoding="utf-8") as f:
        f.write(init_code)
    print("✅ Verified: logic/__init__.py")

if __name__ == "__main__":
    repair_test_page()
    print("\n🚀 Repair Complete. Please refresh your browser.")