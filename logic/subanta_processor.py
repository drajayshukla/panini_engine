...
class SubantaProcessor:
    ...
    def derive_pada(stem_str, vibhakti, vacana, logger=None, force_pratipadika=False):
        ...
        if logger: logger.log("4.1.2", "Svaujasmaut...", f"Selecting '{raw_sup}'", stem, "Maharshi Pāṇini")
        ...
        # Hardcoded Cleaning
        if vibhakti == 1 and vacana == 1: clean_suffix = ad("स्"); rule_applied = "1.3.2 Upadeshe Aj"
        elif vibhakti == 1 and vacana == 2: clean_suffix = ad("औ") 
        elif vibhakti == 1 and vacana == 3: 
            if is_at and is_sarvanama: clean_suffix = ad("ई"); rule_applied = "7.1.17 Jasah Shee"
            else: clean_suffix = ad("अस्"); rule_applied = "1.3.7 Chutu"
        elif vibhakti == 2 and vacana == 1: clean_suffix = ad("अम्")
        elif vibhakti == 2 and vacana == 2: clean_suffix = ad("औ")
        elif vibhakti == 2 and vacana == 3: clean_suffix = ad("अस्"); rule_applied = "1.3.8 Lashakvataddhite"
        elif vibhakti == 3 and vacana == 1: clean_suffix = ad("आ"); rule_applied = "1.3.7 Chutu"
        elif vibhakti == 4 and vacana == 1: clean_suffix = ad("ए"); rule_applied = "1.3.8 Lashakvataddhite"
        elif vibhakti == 5 and vacana == 1: clean_suffix = ad("अस्"); rule_applied = "1.3.8 Lashakvataddhite"
        elif vibhakti == 6 and vacana == 1: clean_suffix = ad("अस्"); rule_applied = "1.3.8 Lashakvataddhite"
        elif vibhakti == 7 and vacana == 1: clean_suffix = ad("इ"); rule_applied = "1.3.8 Lashakvataddhite"
        elif vibhakti == 7 and vacana == 3: clean_suffix = ad("सु"); rule_applied = "1.3.3 Halantyam"
        elif vibhakti == 8 and vacana == 1: clean_suffix = ad("स्"); rule_applied = "1.3.2 Upadeshe Aj"
        elif vibhakti == 8 and vacana == 3: clean_suffix = ad("अस्"); rule_applied = "1.3.7 Chutu"
        ...
        if logger and rule_applied:
            logger.log(rule_applied, "It-Lopa", sanskrit_varna_samyoga(stem + clean_suffix), stem + clean_suffix, "Maharshi Pāṇini")
        
        is_sambuddhi = (vibhakti == 8 and vacana == 1)
        if is_sambuddhi and logger: 
            logger.log("2.3.49", "Ekavacanam Sambuddhih", "Su -> Sambuddhi", stem + clean_suffix, "Maharshi Pāṇini")
        ...
        if is_sambuddhi:
            ...
            if (last in ['ए', 'ओ', 'अ', 'इ', 'उ', 'ऋ']) and clean_suffix:
                if clean_suffix[0].char not in SandhiProcessor.AC:
                    clean_suffix = []
                    if logger: logger.log("6.1.69", "Eng-hrasvat Sambuddheh", f"Deleted 's'", stem, "Maharshi Pāṇini")
        ...
        if is_at:
            if vibhakti == 2 and vacana == 1:
                if clean_suffix and clean_suffix[0].char == 'अ':
                    del clean_suffix[0]
                    if logger: logger.log("6.1.107", "Ami Purvah", sanskrit_varna_samyoga(stem+clean_suffix), stem + clean_suffix, "Maharshi Pāṇini")
                    return SubantaProcessor._finalize(stem + clean_suffix, vibhakti, vacana, logger)
